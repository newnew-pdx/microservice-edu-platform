...
| `edu-learning-service` | 学习服务，使用 Redis Hash、Bitmap、ZSet 实现进度、签到和积分排行 | 8084 |
...
- Redis Hash 学习进度、Bitmap 每日签到和 ZSet 积分排行榜
...
| Step9 | Learning Service：Redis 学习进度、签到与积分排行榜 | 已完成 |
...
Gateway 已增加 `/learning/** -> lb://edu-learning-service` 显式路由。登录取得 token 后，可以访问：

```text
POST /learning/progress/update
GET  /learning/progress/1
POST /learning/sign/today
GET  /learning/sign/month
GET  /learning/rank/top?limit=10
GET  /learning/rank/me
Authorization: Bearer <token>
```

学习进度只写入 Redis Hash；签到由 Lua 原子更新 Bitmap 和 ZSet，不写 MySQL，也不新增 RabbitMQ 业务。
完整 PowerShell 命令、Redis 检查和风险边界见
[docs/step-9-learning-redis.md](docs/step-9-learning-redis.md)。
...
- [Step9：Learning Service + Redis 学习进度、签到与排行榜](docs/step-9-learning-redis.md)
